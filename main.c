 ...
 *
 * NOTE: The library file h has a lot of useful info. Please read it.
 *
...
void ShowReaderDetails() {
	...
	byte v = PCD_ReadRegister1(VersionReg);
	Serial_print(F("MFRC522 Software Version: 0x"));
	Serial_print2(v, HEX);
	if (v == 0x91)
		Serial_print(F(" = v1.0"));
	else if (v == 0x92)
		...
		Serial_print(F(" (unknown)"));
	Serial_println("");
	// When 0x00 or 0xFF is returned, communication probably failed
	if ((v == 0x00) || (v == 0xFF)) {
		Serial_println(F("WARNING: Communication failure, is the MFRC522 properly connected?"));
	}
}

void setup() {
	MFRC522_2(SS_PIN, RST_PIN);	// Create MFRC522 instance
//	Serial.begin(9600);		// Initialize serial communications with the PC
//	while (!Serial);		// Do nothing if no serial port is opened (added for Arduinos based on ATMEGA32U4)
//	SPI.begin();			// Init SPI bus
	PCD_Init1();		// Init MFRC522
	ShowReaderDetails();	// Show details of PCD - MFRC522 Card Reader details
	Serial_println(F("Scan PICC to see UID, type, and data blocks..."));
}
